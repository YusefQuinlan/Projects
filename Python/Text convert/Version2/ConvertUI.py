# -*- coding: utf-8 -*-
"""
Created on Thu Dec 10 11:56:42 2020
...
def save(FileAbsPath,Start,End):
    #IndexError and ValueError to be try-excepted.    -Done 
    # KeyError to be excepted.   -Done but probably should be optimised.
    # start value of 0 gets -1, and if loop skips a page then it gets the
    # next readable page, could lead to index out of bounds
    try:
        ...
        messagebox.showwarning("None existant page numbers",
                               "One or both of the page numbers you have chosen,do not exist" 
                               + "/n Please only use page numbers that exist as start and end values.")
    except ValueError:
        messagebox.showwarning("None number value", "A non-number value has been entered into the start and or\n" +
                               "End value input section, please use numbers for the pagenumbers.")
    except KeyError:
        messagebox.showwarning("Your file may be corrupted", "Some of your pages might be corrupted or otherwise unreadable" 
                               + "You may try again with different page numbers, but please understand that your pdf might not be convertable")
...
"""
def getdir():
     # File not found error to be resolved or try-excepted.
     try:
         ...
         messagebox.showwarning(" Not a valid directory",Dir1 +" is not valid as a directory ")
         Main2.destroy()
